    ...
    public function ajax_select_picktimeAction(){
        if (IS_AJAX) {
            ...
            if((int)(date('w',strtotime($time))) == (int)(date('w',strtotime(date('d-m-Y'))))) {
                if((int)(date('w',strtotime($time)))==6){
                    $resultPickup .='<option value="26" data-value="Noon Collection (12:00AM - 16:00PM)" >Noon Collection (12:00AM - 16:00PM)</option>';
                }else{
                    if(date('H')>=9) {
        ...
        if (IS_AJAX) {
            ...
            // }else{
                ...
                if($this->post('fang')==1){
                    ...
                    $pheves = array("09/08/2018", "22/08/2018", "06/11/2018", "25/12/2018", "01/01/2019");
    ...
    public function ajax_submit2orderinfo_kioskAction(){
        if (IS_AJAX) {
            ...
            if(!$member) {
                ...
                $prefix = strlen($phone) > 10 ? '86' : '65';
                $result = $this->smsApi->sendSms('Your initial Rebox password: '.$password.'. Have a Great Day!', $prefix . $phone);
