...
class Common extends CI_Controller {
    ...
    public function __construct() {
        ...
        //加载物流接口
        $this->roadbul_url = '';
//		set_cookie('roadbul_token',null);die;
        if(get_cookie('roadbul_token')){
            ...
            if($userinfo_array['Code']==0){
                ...
                echo("$userinfo_array[Message]");
            }
//            echo '<script>location.reload();</script>';die;
        }
//        echo get_cookie('roadbul_token');die;
        ...
        $this->admin_pagesize = (isset($this->site['SITE_ADMIN_PAGESIZE']) && $this->site['SITE_ADMIN_PAGESIZE']) ? $this->site['SITE_ADMIN_PAGESIZE'] : 10;
        $this->web_pagesize = (isset($this->site['SITE_WEB_PAGESIZE']) && $this->site['SITE_WEB_PAGESIZE']) ? $this->site['SITE_WEB_PAGESIZE'] : 10;
     ...
     * time   等待时间
     */
    protected function msg($msg, $url = '', $result = 0, $time = 2) {
        $this->view->assign(array(
            'result'    => $result,
            'msg'  => $msg,
            'url'  => $url,
	...
	*/
    ...
    public function get($string) {
        return trim(Controller::get($string));
    }
    ...
    //安全获取参数
    public function safe($param=""){
        return $this->post($param) ? $this->post($param) : $this->get($param);
    }
	
	
	public function oauthlogin(){

		$APPID=$this->inwx_config['appid'];//公众号在微信的appid
		$REDIRECT_URI='http://bl.my8m.com/index.php?c=index&a=xoauth';//回调页面，函数
		$scope='snsapi_base';
