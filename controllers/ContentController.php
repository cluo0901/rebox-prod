...
class ContentController extends Common {
	...
	public function listAction() {
        ...
        $xh_arr = explode(',',$cat['arrchilds']);
        if(in_array($catid,$xh_arr)){
            $this->view->assign('indexc',2);
        }
        $typeid = (int)$this->get('typeid');
            if($typeid){
            $this->view->assign('types',$typeid);
            }else{
                $this->view->assign('types',1);
            }
		...
		if ($cat['child'] == 1 &&$catid!=109&&$catid!=174){
		    //$data = getParentData($catid);
	      $sql = 'select url from ' . $this->content->prefix . 'category where parentid = '.$catid.' order by listorder asc,catid asc limit 1';
