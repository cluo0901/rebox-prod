...
class AttachmentController extends Common {
    ...
    public function __construct() {
		...
		if (!in_array($this->action, array('ajaxswfupload', 'ueditor_upload')) &&
            (!$this->session->is_set('user_id') && !get_cookie('yuangong_id'))
        ) {
//            $this->attMsg(lang('att-13'));
	...
	public function imageAction() {
	    ...
	    if ($this->post('submit')) {
	        ...
	        $data = $this->upload('file', array('jpeg', 'jpg', 'gif', 'png'), $size, '', $mark, $this->post('admin'), null, $this->post('ofile'), $this->post('document'));
            if ($data['result']) {
                $row = array(
                    'msg' => $data['result'],
                    'error' => 1,
                ...
                if($w && $h){
                    $path_thumb = "uploadfiles/admin/" . sha1($data['path']) . ".png";
                    img2thumb($data['path'],$path_thumb,$w,$h);
                    $row = array(
                        'msg' => lang('att-12'),
                        'error' => 0,
                        'filename' => $path_thumb, //缩略图路径
                    );
                }else{
                    $row = array(
                        'msg' => lang('att-12'),
		    ...
		    $w = (int)$this->get('w');
			$h = (int)$this->get('h');
			$s = (int)$this->get('size') ? (int)$this->get('size') : $this->site['FILE_SIZE'];
		    if ($w && empty($h)) {
				$this->view->assign(array(
    ...
    private function upload($fields, $type, $size, $img=null, $mark=true, $admin=0, $stype=null, $ofile=null, $document=null) {
	    ...
	    $upload->set($_FILES[$fields])->set_limit_size(1024*$size)->set_limit_type($type);
        //设置路径和名称
        $ext = $upload->fileext();
        ...
        return array('result' => $result, 'type' => $ext,'path' => $path . $fname, 'file' => $file , 'ext' => $dir == 'image' ? 1 : $ext);
    }
