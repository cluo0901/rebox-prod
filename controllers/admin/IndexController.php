...
class IndexController extends Admin {
	...
	public function indexAction() {
	    ...
	    $this->view->assign('menu', $this->optionMenu());//获取权限不同的菜单

//        if ($this->is_ie()) {
//            $this->view->display('admin/v2_index');
//        } else {
...
return array(
	...
	public function ajaxcountAction() {
        if ($this->get('type') == 'member') {
            ...
            $c2 = $this->content->count('member', 'id', 'status=2');
	...
	public function cityAction() {//市级获取
		$id = (int)$this->post('id');
		if(empty($id)){
		...
		$str = '';
		$list = $this->linkage->get_all_list($id);
		foreach($list as $k=>$v){
